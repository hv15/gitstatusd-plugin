#!/usr/bin/env bash
# Gitstatusd Plugin for Oh-My-Bash
...
[[ -f /usr/share/gitstatus/gitstatus.plugin.sh ]] || return

source /usr/share/gitstatus/gitstatus.plugin.sh

# start the deamon
gitstatus_start GITSTATUS_SH

# redefine the git_prompt_info function, as defined in lib/git.sh
function git_prompt_info() {
    local __prompt
    local __has
    if [[ "$(command git config --get oh-my-bash.hide-status 2>/dev/null)" != "1" ]]; then
        if gitstatus_query GITSTATUS_SH && [[ $VCS_STATUS_RESULT == ok-sync ]]; then
            ...
            if [[ $__has -gt 0 ]]; then
                __prompt="$__prompt$OSH_THEME_GIT_PROMPT_DIRTY"
            else
                __prompt="$__prompt$OSH_THEME_GIT_PROMPT_CLEAN"
            fi
        fi
        echo "$OSH_THEME_GIT_PROMPT_PREFIX${__prompt}$OSH_THEME_GIT_PROMPT_SUFFIX"
    fi
}
